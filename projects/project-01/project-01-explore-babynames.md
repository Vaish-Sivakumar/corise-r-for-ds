...
```
    ...
    #> Error: The pipe operator requires a function call as RHS (<text>:19:1)
...
tbl_names_by_letter |> 
  ...
  labs(
    title = 'Popular Letter by Year',
    subtitle = 'First Letter By Sex',
    x = 'pct_births',
    y = 'first_letter'
  ) +
...
plot_trends_in_letter <- function(my_letter) {
  tbl_names_by_letter |> 
    ...
    labs(
      ...
      subtitle = "Birth Name starts with First Letter",
      caption = "SSA",
      x = "pct_births",
      y = 'year'
    ) +
...
tbl_names_by_first_and_last_letter |> 
  ...
  labs(title = 'Distribution of pct births by first and last letter',
       subtitle = '% of first and last letter by Year' ,
       x = 'last_letter',
       y = 'first_letter') +
...
tbl_names_vowel_consonant |> 
  ...
  facet_wrap(~ sex) +
  # Add labels (title, subtitle, caption, x, y)
  labs(title = 'Baby names in vowels and consonants',
       subtitle = '% of first and letter types by Sex',
       x ='pct_births',
       y = 'year') +
